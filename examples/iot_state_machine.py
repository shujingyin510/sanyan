...
def check_network() -> TriState:
    ...
    if r <= 70: return TriState.TRUE
    if r <= 85: return TriState.MAYBE
    return TriState.FALSE

def check_battery() -> TriState:
    r = random.randint(1, 100)
    if r <= 60: return TriState.TRUE
    if r <= 80: return TriState.MAYBE
    return TriState.FALSE

def check_lock() -> TriState:
    r = random.randint(1, 100)
    if r <= 85: return TriState.TRUE
    if r <= 95: return TriState.MAYBE
    return TriState.FALSE

def check_fingerprint() -> TriState:
    r = random.randint(1, 100)
    if r <= 75: return TriState.TRUE
    if r <= 90: return TriState.MAYBE
    return TriState.FALSE
...
def main():
    ...
    for t in range(1, 16):
        ...
        print(f"  子系统: 网络={net.value} 电池={bat.value} "
              f"锁芯={lock.value} 指纹={fp.value}")
        ...
        if action == "全功能": count_full += 1
        elif action in ("低功耗模式", "离线模式", "降级运行"): count_degraded += 1
        elif action == "安全锁定": count_locked += 1
