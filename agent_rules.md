...
## 规则：代码问答
匹配：Python.*问题|底层.*架构|为什么|怎么实现|原理|设计模式|架构设计|讨论.*代码|技术.*问题|装饰器|GIL|协程|异步|多线程|多进程|垃圾回收|内存.*管理
工具链：
1. done({answer}) — 直接回答技术问题
验证：echo 问答完成

## 规则：项目规划
匹配：规划.*流程|实现.*流程|项目.*方案|开发.*计划|怎么做|如何设计|步骤.*实现|设计.*ER|设计.*系统|设计.*架构
工具链：
1. done({answer}) — 输出项目规划方案
验证：echo 规划完成

## 规则：日常聊天
匹配：你好|谢谢|再见|介绍|随便聊聊|周末|天气|吃饭
工具链：
1. done({answer}) — 日常回复
验证：echo 聊天完成
