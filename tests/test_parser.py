...
class TestSugarParserAST(unittest.TestCase):
    ...
    def test_set(self):
        self.assertAST('设 x = 10', lambda ast: ast[0] == 'set' and ast[1] == 'x' and ast[2] == '10')

    def test_set_short(self):
        self.assertAST('x = 5', lambda ast: ast[0] == 'set' and ast[1] == 'x' and ast[2] == '5')
    ...
    def test_fn(self):
        self.assertAST('定义 平方 (x) { x * x }', lambda ast: ast[0] == 'fn' and ast[1] == '平方' and ast[2] == ['x'])

    def test_lambda(self):
        self.assertAST(
            '设 加倍 = 函数(x) { x * 2 }', lambda ast: ast[0] == 'set' and ast[2][0] == 'lambda' and ast[2][1] == ['x']
        )

    def test_template(self):
        self.assertAST('输出(模板{温度: ${x}°C})', lambda ast: ast[0] == 'print' and ast[1][0] == 'concat')

    def test_judge(self):
        self.assertAST(
            '判 x { 真 { 输出(1) } 可能 { 输出(0) } 假 { 输出(-1) } }', lambda ast: ast[0] == 'judge' and ast[1] == 'x'
        )

    def test_try(self):
        self.assertAST(
            '尝试 { 设 a = 读文件("no.txt") } 捕获 (e) { 输出(e) }', lambda ast: ast[0] == 'try' and ast[2][0] == '捕获'
        )

    def test_read_sensor(self):
        self.assertAST('设 y = 读(人体)', lambda ast: ast[0] == 'set' and ast[2][0] == 'read')
    ...
    def test_fullwidth_string_period(self):
        self.assertAST('输出("测试。")', lambda ast: ast[0] == 'print' and isinstance(ast[1], str) and '。' in ast[1])
