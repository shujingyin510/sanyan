...
_RUNBOOK = (
    ...
    '校验条件或异常类型（外部会逐行核对）；辅助函数**首选定义在模块级**（顶格、类外）'
    '并裸名调用——若放在类里，调用必须写成 类名.方法名(...)（类方法裸名调用必然 '
    'NameError）；不要嵌套在原函数体内——嵌套定义会让原函数更长；'
...
def classify_tip(reason: str, hints: str = '') -> str:
    ...
    if '解析不到的名字' in reason or 'NameError' in reason:
        ...
        return (
            '辅助函数的定义与调用没对上。首选：把辅助函数定义在**模块级**（顶格、类外），'
            '原函数里裸名调用；若定义在类里，调用必须写成 类名.辅助函数名(...)——'
            '类方法裸名调用必然 NameError。两步都落到文件后核对定义位置与调用形式一致。'
        )
