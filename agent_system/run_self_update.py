...
_RUNBOOK = (
    ...
    '或 replace_in_file 小步修改（old 逐字复制、勿含行号前缀，每次控制在 30 行内）；'
    '只搬不改：原函数的每一行原样搬进辅助函数或留在原地，不得改写/删除任何语句、'
    '校验条件或异常类型（外部会逐行核对）；辅助函数推荐在类内新增 @staticmethod'
    '（与原函数同级缩进、紧挨着放），调用写 类名.方法名(evaluator, ...)——'
    '也可放模块级（顶格）用裸名调用；不要嵌套在原函数体内——嵌套定义会让原函数更长；'
...
def classify_tip(reason: str, hints: str = '') -> str:
    ...
    if '嵌套在目标函数内部' in reason:
        ...
        return (
            ...
            '搬到模块级（顶格、类外，与原函数平级），原函数里只留一行裸名调用。'
        )
    if '疑似整段重复粘贴' in reason:
        # 0706 第七轮尝试 3：+390/-0 把整段函数重复贴入——"两步都做完"对这种病不对症
        return (
            '你粘贴了大量重复内容，文件被撑大了。整个改动只需要两笔：插入一个辅助函数定义'
            '（一次），把原块替换成一行调用（一次）——除此之外一行都不该多。'
        )
    if '解析不到的名字' in reason or 'NameError' in reason:
        # 0708 第十六轮：候选把辅助函数定义在类里又裸名调用——"你没定义它"的文案不对症
        # （明明定义了，病在调用形式），候选 2 拿着候选 1 的教训原样重蹈。两条出路点名，
        # 最小修复（补限定前缀）在前——0708 第十九轮回敲：让模型搬模块级会引发定位读循环。
        return (
            '辅助函数的定义与调用没对上。最小修复：若辅助函数定义在类里，把调用改成 '
            '类名.辅助函数名(...)（补上类名前缀即可）；或者把辅助函数移到模块级（顶格、'
            '类外）再裸名调用。核对定义位置与调用形式一致后再 done。'
        )
