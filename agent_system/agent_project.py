...
class ProjectOrchestrator:
    ...
    def _validate(self, task):
        ...
        try:
            r = subprocess.run(
                task.validation.split() if isinstance(task.validation, str) else task.validation,
                capture_output=True,
                text=True,
                timeout=30,
                cwd='.',
            ...
            if r.returncode != 0:
                ...
                if test:
                    parts.append('test=' + test.group(1))
                ev = re.search(r'expected[= ]+(\S+).*?got[= ]+(\S+)', combined, re.I)
                if ev:
                    parts.append('expected=' + ev.group(1) + ' got=' + ev.group(2))
                ae = re.search(r'AssertionError:?\s*(.+)', combined)
                if ae:
                    parts.append('assert=' + ae.group(1)[:200])
                fl = re.search(r'File "(.+?)", line (\d+)', combined)
                if fl:
                    parts.append('file=' + fl.group(1) + ':' + fl.group(2))
            ...
            if a != b:
                d = list(
                    difflib.unified_diff(
                        a.splitlines(), b.splitlines(), fromfile='a/' + f, tofile='b/' + f, lineterm=''
                    )
                )
    ...
    def run(self, spec, workspace='.'):
        ...
        summary = (
            ...
            + str(len(tasks))
            + ' ('
            + str(sum(1 for t in tasks if t.status == TaskStatus.DONE))
            + ' done)'
        )
        print(summary + '\n')
