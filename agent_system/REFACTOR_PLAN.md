...
> **状态（2026-09-10）**：**本文件为历史路线与进度日志，Agent 自更新线已冻结。** S0/S2/S4 落地保留；S1/S3/S5/S6 暂停。解冻条件见知识库 Roadmap / `docs/roadmap.md`。不要把下文 S0–S6 当作当前待办。
...
**进度（2026-07-08 午后③）—— 第二十三轮实跑（淘汰赛⑩）：史上最接近的候选——差一行语义：**

- **候选 3（+8/-1，史上最小 diff 的真候选）两步齐做+调用形式正确+几乎忠实**，通过全部静态四连闸
  **打进 pytest 层**（淘汰赛制下首次；第十三轮的两次在 --attempts 下）。败因是字面上的一行：把兜底
  搬进 helper 后顺手把原函数尾 `return TritValue(0)` "规整"成 `return None`——匹配3 无命中分支时
  返回值变了，4 个 agent 测试点名（与第十三轮同一簇金丝雀：AgentRuntime 初始化链上的 .san 流程）。
- **层次分工按设计工作**：helper 里重新出现一份 `TritValue(0)`，整文件计数不变——守恒 v2 放行是
  **正确**的（它管"行存活"，不管"行在哪"）；这类语义漂移恰好只能由 pytest 层接住 ✓。
  候选 1/2/4 徘徊（7 次超时干净档）。无新机械教训。
- 记账：23 轮 81 次尝试、真候选 27、基建零事故。差距已收敛到"单行语义保真"——继续滚。

**进度（2026-07-08 午后④）—— 第二十四轮实跑（淘汰赛⑪）：2/4 平稳轮：**

- **2/4 候选（4 次超时干净档）**：候选 2 只插入且再现 `return body_node` 改写（丢 `evaluator.eval`，
  未变短先拒）；候选 3 忠实只插入（+77）；候选 1/4 读额徘徊。全部已知类别毫秒拒，无新机械教训。
- 撤案后五轮转化率 9/20=45%（基线 31%）。记账：24 轮 85 次尝试、真候选 29、基建零事故。继续滚。
...
## 实验策略 v2（2026-07-08 定稿）—— 任务工程优先，规律优先于补丁

**背景**：经费有限（两月 ~三四百元）；外部评审两份建议高度一致且被失败库数据背书。
瓶颈排序（评审估计，失败库佐证）：**任务定义过大 40% > 编辑粒度 25% > 上下文组织 20%
> 架构 10% > 模型 5%**——"架构已能稳定管理失败（26 轮零事故），下一步是让模型更容易成功"。

**失败库对排序的量化背书**（`su_stats.py`，88 条记录首报）：
- 模型把"两步任务"当二选一：真候选 53% 纯插入（+73/-0）、23% 纯替换（+3/-74）——
  **任务原子单位超过弱模型单发规划跨度**（"任务定义 40%"的精确版）；
- 死因类 diff 形状签名清晰、第二十三轮最接近候选恰是最小编辑面（+8/-1）——"粒度 25%" 成立；
- runbook 变体转化率 30%→42%→0%→45%——任务书（上下文的一部分）单变量即可翻倍/清零转化率；
- 噪音档与动手率不负相关（风暴 38% vs 干净 29%）——修正旧信念：噪音毁质量（大粘贴），不毁动手率。

**决定**：
1. **停无限轮淘汰赛**（第二十五轮为最后一轮自动续跑）。此后每次实跑必须带假设——
   "一次实验的产出是一条规律，不是一个补丁"。
2. **每周节律**：只新增一个功能；攒 ≥100 条失败库记录；出一份 su_stats 报告；
   REFACTOR_PLAN 进度日志即开发日志。
3. **下一个功能（唯一）：两阶段拆步 edit_fn**——把"94 行重构"拆成两个一动作调用：
   phase A 任务书只说"插入 helper（内容=候选块原文，类内 @staticmethod）"→ 阶段间静态
   检查（helper def 已存在）→ phase B 任务书只说"把 L326-399 替换成一行调用"→ 合并 diff
   走既有 oracle 栈。假设：把"两步当二选一"的原子性问题从模型侧挪到流程侧。
   对照基线：单发转化率 45%（v4）、两步完整率 0/88。
4. **基准集**（第二件）：`benchmark_tasks` 10-20 个任务分层（小 20-40 行 / 中 40-80 / 大 80+，
   跨文件），`su_bench` 驱动器复用 run_self_update + su_stats；harness 改动先跑小层冒烟。
5. **oracle 当数据源**：失败库按层出拒绝分布（已有）；定期审误拒面（守恒 ≥8 字符阈值等）。
6. **不做**：训练/微调自有模型；无假设的 in-loop 反制堆叠。

**与 S0-S6 的关系**：S0/S2/S4 已落地；S1（首胜流程）/S3（failing_test）触发条件不变；
S5/S6 吸收进本节奏（su_stats 即 S6 的统计件）。

**进度（2026-07-08 下午）—— 第二十五轮实跑（淘汰赛⑫，最后一轮自动续跑）+ 拆步执行器落地（`fc955f3`）：**

- **第二十五轮 2/4（6 次超时干净档，4/4 全跑完无断路）**：两个真候选（未变短/解析不到类）、两个徘徊，
  全部毫秒层拒。按实验策略 v2 与用户预算指示，**此轮为最后一轮无假设自动续跑**——失败库刷新至
  92 条记录（25 轮全量入库）。累计：25 轮 92 次尝试、真候选 32、0 接受、基建零事故。
- **拆步执行器（`--staged`）落地**：make_staged_edit_fn + make_def_exists_check + build_stage_plans
  ——固定两步计划（步1只插入 → 阶段间静态检查 → 步2只替换），假设与对照基线已入库；
