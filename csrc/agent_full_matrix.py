...
for tid, task, category in tests:
    ...
    results.append({
        "id": tid, "task": task[:60], "cat": category,
        "rule": rule_hit, "llm": llm_calls, "ternary": ternary_count,
        "ur_min": round(ur_min, 3), "ur_stop": ur_stop, "negate": negate,
        "crashed": crashed, "done": done, "time": f"{dt:.1f}s",
    })
...
# ── 统计 ──
print(f"\n{'='*60}")
print(f"组  规则覆盖 LLM调用  UR最低  UR拦截  存活  说明")
print("-" * 60)
