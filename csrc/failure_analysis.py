...
import torch, numpy as np, json, time, urllib.request, re, random
...
test_prompts = [
    ...
    ("Once upon a time", "The cat cat", "They went to boy", "She went to boy",
     "A big dog ran through", "The little house", "I like to play with"),
...
for prompt in all_test:
    inputs = tokenizer(prompt, return_tensors='pt'); plen = inputs['input_ids'].shape[1]
    with torch.no_grad():
        o = model.generate(**inputs, max_new_tokens=40, do_sample=True,
                           temperature=0.8, top_p=0.9, top_k=0, pad_token_id=tokenizer.eos_token_id)
    ids = o[0].tolist()
    min_ur = 1.0; trigger_step = -1
    for pos in range(plen + 8, len(ids)):
        r = ids[max(0, pos - 32):pos]
        ur = len(set(r)) / max(len(r), 1)
        if ur < min_ur: min_ur = ur
...
print(f"  UR < 0.30: {sum(1 for x in human_ur if x < 0.30)}/{len(human_ur)} ({sum(1 for x in human_ur if x < 0.30)/len(human_ur)*100:.1f}%)")
print(f"  UR < 0.40: {sum(1 for x in human_ur if x < 0.40)}/{len(human_ur)} ({sum(1 for x in human_ur if x < 0.40)/len(human_ur)*100:.1f}%)")
...
with open("benchmarks/human_baseline_extended.json", "w") as f:
    json.dump({"n_passages": len(samples), "n_ur_samples": len(human_ur),
               "mean": float(np.mean(human_ur)), "std": float(np.std(human_ur)),
               "min": float(min(human_ur)), "below_03": int(sum(1 for x in human_ur if x < 0.30)),
               "below_04": int(sum(1 for x in human_ur if x < 0.40))}, f, indent=2)
