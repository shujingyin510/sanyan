    ...
    def check(self, ids, enc, sbert, window=32):
        ...
        if len(ids) >= 24 and len(ids) % 8 == 0:
            ...
            if past and recent:
                ...
                sim = float(np.dot(emb_recent, emb_past) / (np.linalg.norm(emb_recent) * np.linalg.norm(emb_past) + 1e-8))
...
# ── 测试 ──
np.random.seed(42)
prompts = ["Once upon a time there was a little girl who",
...
print(f"Combined: {len([1 for u,s in zip(detector.ur_history[-len(detector.sim_history):], detector.sim_history) if u > 0.30 and s > 0.85])} semantic-only loops (UR masked)")

os.makedirs("benchmarks", exist_ok=True)
with open("benchmarks/dual_channel.json", "w") as f:
    json.dump({"ur_history": [float(x) for x in detector.ur_history],
               "sim_history": [float(x) for x in detector.sim_history]}, f, indent=2)
