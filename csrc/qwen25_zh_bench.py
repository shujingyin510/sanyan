...
zh_template = [
    "从前有一个","小男孩在","一只大狗","今天天气","他走进了",
    "她看着窗外","那座山上","小明每天","老师说","春天来了",
    "我喜欢","妈妈做的","爸爸告诉","学校里","夜晚的星空",
    "小猫在","那本书","河水很","远处的","昨天的",
    "他决定","她突然","这座城市的","我们一起去","他画了一幅",
    "她唱歌","火车","篮球场上","冰箱里有","花园里",
    "他打开手机","天黑了","海边","周末","考试前",
    "他梦见","她收到一封","那条路","爷爷说过","童年的",
    "每当","虽然","因为他","如果明天","只要努力",
    "在那个遥远的","一阵风吹过","雨后","阳光洒在","深夜里",
]
zh_extra = [
    "故事","路上","花开了","很冷","教室",
    "菜很好吃","信","一个秘密","很热闹","很美",
...
prompts = [f"{zh_template[i%len(zh_template)]}{zh_extra[(i//len(zh_template))%len(zh_extra)]}" for i in range(1000)]

UR_TH = 0.30; fp = 0; ur_mins = []; t0 = time.perf_counter()
for i, p in enumerate(prompts):
    inputs = tokenizer(p, return_tensors='pt'); plen = inputs['input_ids'].shape[1]
    with torch.no_grad():
...
print(f"\n=== 中英对比 ===")
print(f"  英文 (1000 prompt): FP=4/1000 (0.4%), avg min_UR=0.717")
