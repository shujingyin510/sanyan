...
def llm_call(prompt):
    ...
    try:
        with urllib.request.urlopen(req,timeout=10) as r:
            ...
            dt = time.time() - t0
            # 分类统计
            if '对话语气' in prompt: cat = '语气检测'
            elif '语气是' in prompt: cat = '行为分类'
            elif '回应' in prompt or '顶回去' in prompt or '接话' in prompt: cat = 'NPC回应'
            elif '对' in prompt and '说句话' in prompt: cat = 'NPC说话'
            else: cat = '其他'
            _llm_times[cat] = (_llm_times.get(cat, [0, 0.0])[0] + 1,
                               _llm_times.get(cat, [0, 0.0])[1] + dt)
            # 去所有引号
...
def _gen_dialogue(ev, args):
    ...
    d1=npc_data.get(n1,{}) if isinstance(npc_data, dict) else {}; d2=npc_data.get(n2,{}) if isinstance(npc_data, dict) else {}
    ...
    pers1 = d1.get('性格',''); pers2 = d2.get('性格','')
    # 根据天气和关系决定对话语气方向
    ...
    if hasattr(existing_trust, 'to_int'): existing_trust = float(existing_trust.to_int())
    ...
    try:
        ...
        if isinstance(_npc_pos, dict):
            p1 = _npc_pos.get(n1, ''); p2 = _npc_pos.get(n2, '')
            if hasattr(p1, 'to_payload'): p1 = str(p1.to_payload())
            if hasattr(p2, 'to_payload'): p2 = str(p2.to_payload())
            if p1 and p2 and p1 == p2: loc_mult = 1.2
            elif p1 and p2: loc_mult = 0.6
    except: pass
    # #1 事件记忆：注入近期事件到对话提示
    event_ctx = ''
    for pd in sorted(_event_memory.keys(), reverse=True):
        if pd == '_chain': continue
        for evt in _event_memory.get(pd, []):
            if n1 in evt or n2 in evt:
                event_ctx += f'前几天:{evt};'
                if len(event_ctx) > 150: break
        if len(event_ctx) > 150: break
    ...
    p1 = (f'{n1}是{role1}，性格{pers1}，正在{act1}。天气{weather}。'
          f'对{n2}({role2}){tone_prompt}。{event_hint}要符合{role1}身份，10-30字。只输出这句话。')
    line1 = llm_call(p1)
    line2 = None  # 防御：LLM 返回 None 时不报 UnboundLocalError
    if line1:
        cache[n1] = line1
        reply_tone = {'友好':'友善回应','打趣':'接话逗回去','抱怨':'附和或劝解','平淡':'随口接话','冲突':'反驳或顶回去'}.get(tone, '回应')
        p2 = (f'{n2}是{role2}，性格{pers2}，正在{act2}。{n1}对你说："{line1}"。'
    ...
    # ── 行为标签分析 ──
    behave_delta = {
        '帮助':0.060, '赞扬':0.030, '交易':0.015, '问候':0.009,
        '争吵':-0.090, '欺骗':-0.240, '赠礼':0.150, '闲聊':0.003
    }
    ...
    if full_text:
        ...
        for kw in ['骗', '撒谎', '胡说', '胡扯', '瞎说', '滚', '偷', '抢', '凭什么',
                     '咋回事', '怎么搞的', '你啥意思', '管得宽']: 
            if kw in full_text: label = '争吵'; break
        for kw in ['送你', '拿着', '收下', '给你带']: 
            if kw in full_text and label == '问候': label = '赠礼'; break
        # LLM 分类：关键词未命中时用大模型判断对话语气
        if label == '问候' and has_llm:
            cls = llm_call(
                f'"{line1}" "{line2}" 这段对话是：问候、交易、争吵、帮助、赞扬、赠礼、闲聊？只输出一个词。')
            if cls and cls in behave_delta:
                label = cls

    # ── 动态 δ 加权：性格 + 天气 + 对话长度 ──
    p1 = d1.get('性格',''); p2 = d2.get('性格','')
    ...
    _social_activity[n1][0] += 1; _social_activity[n1][1] += delta
    _social_activity[n2][0] += 1; _social_activity[n2][1] += delta
    ...
    if _verbose and line1 and line2:
        ...
        cache['_trit'] = (
            ...
            f'  │ 行为判定: {label} ({src})'
            + (f'\n  │ 加权因子: {weight_str}' if weight_str else '') +
            formula + '\n'
    ...
    if label != '问候' and label != '闲聊':
        ...
        _event_memory.setdefault('_chain', []).append(
            {'day': d, 'type': label, 'n1': n1, 'n2': n2, 'delta': delta})
    return TritValue(0)
...
_neg_events = [
    ('鸡进了{0}的菜地，踩坏了好几棵菜。', '菜地被踩', -0.060, True,
     {0: ['老农','农妇']}),  # 只有种菜的才有菜地
    ...
    ('{0}说{1}卖东西缺斤短两，{1}很生气。', '生意纠纷', -0.080, True,
     {1: ['小贩','铁匠']}),  # 只有做生意的才卖东西
    ('{0}借了{1}的钱一直没还，今天被当面问了。', '借钱不还', -0.100, False, None),
    ('{0}和{1}因为地界的事吵了一架。', '地界纠纷', -0.060, True,
     {0: ['老农','农妇'], 1: ['老农','农妇']}),  # 地界纠纷只有种地的之间有
]
...
def _night_events(ev, args):
    ...
    if r < 25: print('  远处传来几声狗叫。')
        ...
        if keys: print(f'  {random.choice(keys)} 半夜起来喂了趟牲口。')
    elif r < 8:
        nd = ev.get_var('NPC数据') if ev.has_var('NPC数据') else {}
        if hasattr(nd, 'is_dict') and hasattr(nd, 'to_payload') and nd.is_dict():
            nd = nd.to_payload()
    ...
    if random.random() < 0.25:
        ...
        if len(npc_keys) >= 2:
            ...
            for evt in candidates:
                ...
                if role_filter:
                    ...
                    for k in npc_keys:
                        ...
                        if 0 in role_filter and role not in role_filter[0]: continue
                        if 1 in role_filter and role not in role_filter[1]: continue
                        valid.append(k)
            ...
            for n in [n1, n2]:
                ...
                # 冲突类事件映射到争吵系数
                mapped = {'菜地被踩':'争吵','借物不还':'争吵','传错话':'争吵',
                          '谣言':'争吵','家畜纠纷':'争吵','生意纠纷':'争吵',
                          '借钱不还':'争吵','地界纠纷':'争吵'}.get(behav, behav)
            ...
            if _verbose:
                rel = '邻居'
                try: rel = str(ev.eval(['取关系', n1, n2]))
                except: pass
                if new >= 0.7: zone = '真 ●●●'
                elif new >= 0.3: zone = '可能 ◐◐◐'
                else: zone = '假 ○○○'
                try:
                ...
                except: period = '深夜'; weather = ''
                print(f'  ┌─ 第{_v["天数"]}天 {period} {weather} 夜间事件')
                print(f'  │ 行为判定: {event[1]} (冲突池)')
                if mult_p_night != 1.0:
                    print(f'  │ Δ = {event[2]:+.3f}(固定) × 性格{mult_p_night:.2f}({",".join(mult_parts)}) = {delta_e:+.3f}')
                else:
...
class Tee:
    def write(self, s):
        ...
        orig.write(out); log.write(out); log.flush()
...
COL_W = 8  # 每列视觉宽度
header = _vpad('', 16)
for b in all_behaviors: header += _vpad(b, COL_W)
print(header)
...
try:
    for day in range(max_days):
        ...
        for v in td_now.values():
            try: vals.append(float(v.to_int()) if hasattr(v, 'to_int') else float(v))
            except: pass
        if vals:
            ...
            print(f'  凝聚力: {avg:.3f}(全部{len(vals)}对平均)  活跃:{active_avg:.3f}(已互动{len(active_vals)}对)  假:{false_n} 可能:{maybe_n} 真:{true_n}')
        # #3 每日矩阵（verbose 模式）
                    ...
                    for n1 in all_n:
                        ...
                        for n2 in all_n:
                            if n1 == n2:
                                ...
                                try: v = float(v_raw.to_int()) if hasattr(v_raw, 'to_int') else float(v_raw)
                                except: v = 0.0
                                row += _vpad(f'{v:.2f}', 7)
                        print(row)
                    print('  └─')
    ...
    if _trust_timeline and len(_trust_timeline) >= 2:
        def _to_f(v):  # 浮点保护：处理 TritValue 或 int
            try:
                if hasattr(v, 'to_int'): return float(v.to_int())
                return float(v)
            except: return 0.0
        ...
        # 末天新出现 / 首天后消失的关系（同一列宽）
        if new_keys:
            for k in sorted(new_keys): print(f'  {k:<{max_k_len}} 新增={_to_f(last[k]):.3f}')
        if gone_keys:
            for k in sorted(gone_keys): print(f'  {k:<{max_k_len}} 消失 (原{_to_f(first[k]):.3f})')
        # 统计：仅按 common 键计算总变动
        ...
        print(f'  共 {days} 天，{len(common)} 组持续关系' + (f'（+{len(new_keys)} 新增/-{len(gone_keys)} 消失）' if (new_keys or gone_keys) else ''))
        ...
        if event_days:
            ...
            for k, events in pairs.items():
                if len(events) >= 2:
                    if not has_causal: print(); print('══ 因果链 ══'); has_causal = True
                    n1, n2 = events[0]['n1'], events[0]['n2']
                    chain_str = ' → '.join(f'D{e["day"]}:{e["type"]}' for e in sorted(events, key=lambda x: int(x['day'])))
        ...
        if _trust_timeline:
            ...
            if len(all_names) >= 2:
                ...
                M_W = 9  # 每列视觉宽度
                header = _vpad('', 10)
                for n in all_names: header += _vpad(n[:4], M_W)
                print(header)
                for n1 in all_names:
                    row = _vpad(n1[:6], 10)
                    for n2 in all_names:
                        if n1 == n2:
                            ...
                            if v >= 0.7: mark = f'{v:.2f}●'
                            elif v >= 0.3: mark = f'{v:.2f}◐'
                            else: mark = f'{v:.2f}○'
                            row += _vpad(mark, M_W)
                    print(row)
                print('════════════')
        ...
        if len(all_days) >= 2 and pairs:
            ...
            for pair, vals in sorted(pairs.items()):
                if len(vals) < 2 or max(vals) - min(vals) < 0.002: continue
            ...
            if active_pairs:
                ...
                colors = ['#e74c3c','#3498db','#2ecc71','#9b59b6','#f39c12','#1abc9c',
                           '#e67e22','#34495e','#e91e63','#00bcd4','#4caf50','#ff9800']
                ...
                if y_max - y_min < 0.1: y_min, y_max = 0, 0.3  # 数据太集中时放大到 0~0.3
                y_range = y_max - y_min
                svg_parts = [
                ...
                # 三态背景色带（按数据范围映射）
                for (lo, hi, color, label) in [(0, 0.3, '#ff444422', '假'), (0.3, 0.7, '#ffaa0022', '可能'), (0.7, 1.0, '#44ff4422', '真')]:
                    if hi <= y_min or lo >= y_max: continue
                    b_lo = pad_t + plot_h * (1 - min(hi, y_max) / y_max)
                    b_hi = pad_t + plot_h * (1 - max(lo, y_min) / y_max)
                    if b_hi - b_lo < 1: continue
                    svg_parts.append(f'<rect x="{pad_l}" y="{b_lo:.0f}" width="{plot_w}" height="{b_hi-b_lo:.0f}" fill="{color}"/>')
                    svg_parts.append(f'<text x="{pad_l+5}" y="{b_lo+14:.0f}" fill="{color[:7]}88" font-size="10">{label}</text>')
                # 网格
                for i in range(5):
                    y_val = y_min + i * y_range / 4
                    y = pad_t + plot_h * (1 - (y_val - y_min) / y_range)
                    svg_parts.append(f'<line x1="{pad_l}" y1="{y:.0f}" x2="{w-pad_r}" y2="{y:.0f}" stroke="#ffffff15"/>')
                    svg_parts.append(f'<text x="{pad_l-8}" y="{y+4:.0f}" fill="#aaa" font-size="10" text-anchor="end">{y_val:.2f}</text>')
                for di, d in enumerate(days):
                    x = pad_l + plot_w * di / max(n_days - 1, 1)
                    svg_parts.append(f'<text x="{x:.0f}" y="{h-pad_b+18}" fill="#aaa" font-size="10" text-anchor="middle">D{d}</text>')
                # 折线 + 交互悬停
                legend_items = []
                for ci, (pair, vals) in enumerate(active_pairs):
                    ...
                    svg_parts.append(f'<polyline points="{" ".join(pts)}" fill="none" stroke="{color}" stroke-width="2.5"/>')
                    for di, v in enumerate(vals):
                        x = pad_l + plot_w * di / max(n_days - 1, 1)
                        y = pad_t + plot_h * (1 - (v - y_min) / y_range)
                        svg_parts.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="5" fill="{color}" stroke="#fff" stroke-width="1">')
                    ...
                    svg_parts.append(f'<text x="{x_last+5:.0f}" y="{y_last+4:.0f}" fill="{color}" font-size="11">{pair}</text>')
                ...
                for ci, (gid, color, pair) in enumerate(legend_items):
                    ...
                    svg_parts.append(f'<label xmlns="http://www.w3.org/1999/xhtml" style="color:{color};font-size:11px;cursor:pointer;white-space:nowrap">')
                    svg_parts.append(f'<input type="checkbox" id="{cb_id}" checked onchange="document.getElementById(\'{gid}\').style.display=this.checked?\'block\':\'none\'" style="accent-color:{color};vertical-align:middle"/>')
                    svg_parts.append(f' {pair}</label></foreignObject>')
                # 标题
                svg_parts.append(f'<text x="{w/2:.0f}" y="20" fill="#fff" font-size="16" text-anchor="middle">桃花村 {max_days}天 互信演化</text>')
