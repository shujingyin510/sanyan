...
对 UR 在 [0.15, 0.45] 区间做滑动扫描，计算每个阈值下的 TPR/FPR/F1/Youden's J：

| 阈值 | TPR | FPR | Precision | F1 | Youden's J |
|------|-----|-----|-----------|----|-----------|
| 0.20 | 1.000 | 0.000 | 1.000 | 1.000 | 1.000 |
| 0.25 | 1.000 | 0.000 | 1.000 | 1.000 | 1.000 |
| **0.30** | **1.000** | **0.000** | **1.000** | **1.000** | **1.000** |
| 0.35 | 1.000 | 0.000 | 1.000 | 1.000 | 1.000 |
| 0.40 | 1.000 | 0.000 | 1.000 | 1.000 | 1.000 |
| 0.45 | 1.000 | 0.001 | 0.999 | 1.000 | 0.999 |

> 在 0.20–0.40 范围内，阈值的区分能力完全相同（TPR=1.0, FPR=0）。这说明**两类分布在 0.20–0.40 之间存在一个宽分离带，阈值选择在这个区间内的任意值都等效**。我们选择 0.30 而非中点 0.402（(0.704+0.101)/2），是更保守的策略——更低的阈值意味着更严格的退化判定，减少假阳性风险。
