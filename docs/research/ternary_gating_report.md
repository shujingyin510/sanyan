...
|------|---|--------|------------|------------|
| 英文 | 50 | 1 (2%) | 0.692 | 0.219 |
| **中文** | **50** | **1 (2%)** | **0.718** | 0.188 |

> 中文 UR 略高（0.718 vs 0.692）——Qwen2.5 在母语中文上多样性更强。唯一触发案例为"小猫在"（prompt 过短且语法不完整，导致模型循环补全）。**0.30 阈值在跨语言场景下保持一致的误报率（2%），且均为输入导致而非模型内部退化。**
### 核心结论
