...
class HighlightText(tk.Frame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self._after_id = None
        # 行号
        self.lineno = tk.Text(self, width=5, padx=4, pady=4, wrap='none',
                              font=('Consolas', 11), takefocus=0,
                              borderwidth=1, relief='sunken',
                              bg='#f0f0f0', fg='#808080', state='disabled')
        self.lineno.pack(side='left', fill='y')
        # 断点标示条
        self.bp_bar = tk.Text(self, width=2, padx=2, pady=4, wrap='none',
                              font=('Consolas', 11), takefocus=0,
                              borderwidth=1, relief='sunken',
                              bg='#f0f0f0', fg='red', state='disabled',
                              cursor='hand2')
        ...
        self.text = tk.Text(self, wrap='none', font=('Consolas', 11),
                            undo=True, tabs=4, padx=4, pady=4,
                            borderwidth=1, relief='sunken',
...
class FindReplace(tk.Frame):
    ...
    def __init__(self, master, editor):
        ...
        self.find_entry = tk.Entry(self, textvariable=self.find_var, width=25,
                                   font=('Consolas', 10))
        ...
        tk.Button(self, text='↓', command=self.find_next, width=3,
                  bg='#e8e8e8', relief='raised', bd=1).pack(side='left', padx=1)
        tk.Button(self, text='↑', command=self.find_prev, width=3,
                  bg='#e8e8e8', relief='raised', bd=1).pack(side='left', padx=1)
        tk.Label(self, text='替换:', bg='#e8e8e8').pack(side='left', padx=4)
        self.replace_var = tk.StringVar()
        tk.Entry(self, textvariable=self.replace_var, width=15,
                 font=('Consolas', 10)).pack(side='left', padx=2)
        tk.Button(self, text='替换', command=self.replace_one, width=4,
                  bg='#e8e8e8', relief='raised', bd=1).pack(side='left', padx=1)
        tk.Button(self, text='全部替换', command=self.replace_all, width=7,
                  bg='#e8e8e8', relief='raised', bd=1).pack(side='left', padx=1)
        tk.Button(self, text='✕', command=self.hide, width=2,
                  bg='#e8e8e8', relief='flat', bd=0, font=('Segoe UI', 8)).pack(side='right')
...
class SanyanIDE:
    ...
    def _make_toolbar(self, parent):
        ...
        s = {'relief': 'raised', 'bd': 1, 'bg': '#e8e8e8',
        ...
        self._step_btn = tk.Button(tb, text='↘ 单步', command=self._debug_step,
                                   state='disabled', **s)
        self._step_btn.pack(side='left', padx=2, pady=2)
        self._cont_btn = tk.Button(tb, text='▶ 继续', command=self._debug_continue,
                                   state='disabled', **s)
    ...
    def _build_ui(self):
        ...
        ttk.Label(left_frame, text='  项目文件', font=('微软雅黑', 9, 'bold'),
        ...
        for key, cmd in [('<Control-R>', self._run_code), ('<Control-r>', self._run_code),
                         ('<Control-s>', self._save_file), ('<Control-S>', self._save_file),
                         ('<Control-o>', self._open_file), ('<Control-O>', self._open_file),
                         ('<Control-n>', self._new_file), ('<Control-N>', self._new_file),
                         ('<Control-f>', self._show_find), ('<Control-F>', self._show_find),
                         ('<F5>', self._run_code), ('<F6>', self._debug_run),
                         ('<F10>', self._debug_step), ('<F8>', self._debug_continue)]:
        ...
        self.output_text = scrolledtext.ScrolledText(
            out_frame, wrap='word', font=('Consolas', 10),
        ...
        self.status_bar = tk.Label(self.root, text='  就绪', relief='sunken',
                                   anchor='w', bg='#e8e8e8', fg='black')
            ...
            if ast is None:
                ...
                if tokens:
                    try:
                        ...
                        self._log(f'[错误] 语法解析失败\n')
                        return
            if ast is None:
                self._write_output('代码为空\n')
                return
            self._log(f'[运行] AST 就绪\n')
            result = env.eval(ast)
            output = buf.getvalue()
            if output:
