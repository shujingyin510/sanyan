...
OP_TO_INSTR = {
    ...
    'not': 'NOT', 'wait': 'WAIT',
    'io_write': 'IO_WRITE', 'io_read': 'IO_READ',
    '加': 'ADD', '减': 'SUB', '乘': 'MUL', '除': 'DIV', '余': 'MOD',
    '等于': 'EQ', '不等': 'NE', '大于': 'GT', '小于': 'LT', '大等': 'GTE', '小等': 'LTE',
    '非': 'NOT', '等待': 'WAIT',
    'io写': 'IO_WRITE', 'io读': 'IO_READ',
    '输出': 'PRINT',
}
...
def compile_ast(node, bc: BytecodeWriter, ctx: CompileContext):
    """递归编译 AST 节点为字节码。"""
    # 字面量
    ...
    if op in ('loop', '循环'):
        ...
        struct.pack_into('<h', bc.data, jz_pos + 1,
                         len(bc.data) - jz_pos - 3)  # 跳过体+JMP
        return
...
def resolve_fixups(bc: BytecodeWriter, ctx: CompileContext):
    """回填前向引用函数地址。"""
